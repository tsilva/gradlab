...
def local_wandb_writer(run_dir: Path, config: dict, *, backfill: bool = False):
    ...
    if not backfill and not wandb_publication_enabled(config):
        yield None
        return
    with (run_dir / ".wandb-writer.lock").open("a") as lease:
        try:
        ...
        store = MetricStore(run_dir / "gradlab.sqlite")
        store.init()
        ...
        projector = WandbProjector.start_live(
            config,
            run_dir=str(run_dir),
            config=resolve_env_config(env_config_from_mapping(config)),
        ...
        def publish() -> None:
            try:
                while not finished.is_set():
                    publish_pending_frames(store, run, limit=100)
                    finished.wait(0.5)
        ...
        try:
            ...
            try:
                if errors:
                    raise RuntimeError("local W&B publisher failed") from errors[0]
                deadline = time.monotonic() + 60
                while store.pending_metric_frames(limit=1):
                    count = publish_pending_frames(store, run, limit=100)
                ...
                if result_file.is_file():
                    ...
                    run.summary["ops/state"] = result["status"]
                    run.summary["ops/reason"] = result["terminal_reason"]
                with store.connection() as connection:
                    high_water = int(
                        connection.execute(
                ...
                print(
                    "W&B delivery also failed; saved metrics require synchronization.", flush=True
                )

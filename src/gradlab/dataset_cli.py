...
def _dataset_root_arg(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Local dataset root (default: ~/.gradlab/datasets).",
    )


def _record(args: argparse.Namespace) -> int:
    from gradlab.dataset_record import record_command

    return record_command(args)


def _list(args: argparse.Namespace) -> int:
    from gradlab.dataset_store import list_command

    return list_command(args)


def _play(args: argparse.Namespace) -> int:
    from gradlab.dataset_store import play_command

    return play_command(args)


def _verify(args: argparse.Namespace) -> int:
    from gradlab.dataset_store import verify_command

    return verify_command(args)


def _video(args: argparse.Namespace) -> int:
    from gradlab.dataset_store import video_command

    return video_command(args)


def _upload(args: argparse.Namespace) -> int:
    from gradlab.dataset_hub import upload_command

    return upload_command(args)


def _adopt(args: argparse.Namespace) -> int:
    from gradlab.dataset_store import adopt_command

    return adopt_command(args)


def _export_minari(args: argparse.Namespace) -> int:
    from gradlab.dataset_minari import export_minari_command

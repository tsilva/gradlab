...
PRODUCER_PREFIX = "gradlab:"
RUNTIME_COLUMNS = frozenset({"_gradlab_dataset_root", "_gradlab_hf_repo_id"})
MAX_INFO_BYTES = 1 << 20
...
def feature_identity(features: Any) -> str:
    payload = canonical_json_bytes(
        {
            "domain": "gradlab.dataset.features",
            "version": 1,
...
def episode_content_fingerprint(
    ...
    *,
    ...
    hasher.update(b"gradlab.dataset.episode\x00v2\x00")
    for row in rows:
        logical = {
            key: value
...
def collection_fingerprint(features: Any, episode_fingerprints: Iterable[str]) -> str:
    hasher = hashlib.sha256()
    hasher.update(b"gradlab.dataset.collection\x00v2\x00")

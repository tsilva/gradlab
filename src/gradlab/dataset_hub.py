...
def materialize_hub_source(source: str) -> MaterializedHubSource:
    try:
        ...
        raise RuntimeError("Hub dataset support requires the GradLab dataset extra") from exc
    ...
    temporary = tempfile.TemporaryDirectory(prefix="gradlab-hf-dataset-")
...
class HubAppendSession:
    ...
    def __init__(self, target: str, *, revision: str = "main") -> None:
        try:
            ...
            raise RuntimeError("Hub dataset upload requires the GradLab dataset extra") from exc
        ...
        if exists:
            ...
            try:
                ...
                sidecar = snapshot / "gradlab-sessions.json"
                if sidecar.is_file():
                    document = json.loads(sidecar.read_text(encoding="utf-8"))
                    if document.get("version") != 1 or not isinstance(
                        document.get("gradlab_sessions"), list
                    ):
                        raise ValueError("remote GradLab session sidecar is unsupported")
                    self.sessions = list(document["gradlab_sessions"])
            finally:
                remote.cleanup()
        else:
    ...
    def append(self, source: Path) -> str:
        ...
        with tempfile.TemporaryDirectory(prefix="gradlab-hf-commit-") as temporary_name:
            temporary = Path(temporary_name)
            shard = temporary / f"gradlab-{uuid.uuid4().hex}.parquet"
            ...
            sidecar = temporary / "gradlab-sessions.json"
            session_entry = {
            ...
            sidecar.write_text(
                json.dumps(
                    {
                        "version": 1,
                        "gradlab_sessions": [*self.sessions, session_entry],
                    },
                    sort_keys=True,
                    indent=2,
                ),
            ...
            operations.append(
                CommitOperationAdd(path_in_repo="gradlab-sessions.json", path_or_fileobj=str(sidecar))
            )
            try:
                result = self.api.create_commit(
                    ...
                    commit_message=f"Append {len(new_episode_ids)} verified GradLab episode(s)",
                )
            except Exception:
...
def upload_command(args: Any) -> int:
    from gradlab.dataset_store import open_source

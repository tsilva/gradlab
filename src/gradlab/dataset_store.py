...
from gradlab.dataset_media import iter_episode_frames, iter_selected_frames
from gradlab.file_utils import atomic_write_json, fsync_path, fsync_tree


COLLECTION_IDENTITY_VERSION = 1
TRANSACTION_VERSION = 1
DEFAULT_DATASET_ROOT = Path.home() / ".gradlab" / "datasets"
...
def stage_dataset_tree(source: Path) -> StagedDatasetTree:
    ...
    temporary = tempfile.TemporaryDirectory(prefix="gradlab-dataset-stage-")
...
def open_source(source: str, *, root: Path | None = None) -> LoadedDataset:
    ...
    if source.startswith("hf://"):
        from gradlab.dataset_hub import materialize_hub_source
...
def list_command(args: Any) -> int:
    ...
    if args.reference:
        try:
            ...
            from gradlab.dataset_hub import HubFeatureGroupsError
...
def verify_command(args: Any) -> int:
    with open_source(args.source, root=args.root) as loaded:
        if args.reexecute:
            from gradlab.dataset_record import reexecute_dataset
...
def play_command(args: Any) -> int:
    from gradlab.play_web import run_web_dataset_playback

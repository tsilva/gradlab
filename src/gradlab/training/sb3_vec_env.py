    ...
    def step_wait(self):
        ...
        if np.any(self._dones):
            ...
            for lane in np.flatnonzero(self._dones):
                ...
                episode_return = float(info.pop("gradlab_episode_return"))
                episode_length = int(info.pop("gradlab_episode_length"))
                episode_elapsed = float(info.pop("gradlab_episode_elapsed"))
                control_boundary = bool(info.pop("gradlab_control_boundary", False))

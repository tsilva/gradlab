...
class BatchRuntime:
    ...
    def step(self, actions: Any) -> BatchStep:
        ...
        if done_indices.size:
            ...
            for name, values in (
                ("gradlab_episode_return", self._episode_returns.copy()),
                ("gradlab_episode_length", self._episode_lengths.copy()),
                (
                    "gradlab_episode_elapsed",
                    np.full(
                        self.num_envs,
            ...
            owned_transition_info["gradlab_boundary_reason"] = boundary_reasons
            owned_transition_info["_gradlab_boundary_reason"] = dones.copy()
            owned_transition_info["gradlab_reset_reason"] = reset_reasons
            owned_transition_info["_gradlab_reset_reason"] = forced_only_mask.copy()
            owned_transition_info["gradlab_control_boundary"] = control_boundaries.copy()
            owned_transition_info["_gradlab_control_boundary"] = dones.copy()

...
export function createChartHistory({ token = "", request = chartRequest(token), onChange = () => {}, clock = {
  ...
  function read() {
    const inputs = [data, context.liveHistory, context.episode, context.throughStep,
      range, status, error];
    if (readResult && inputs.every((value, index) => value === readInputs[index])) return readResult;
    readInputs = inputs;
    // Snapshot and history callbacks share this view until its contents change.
    readResult = {

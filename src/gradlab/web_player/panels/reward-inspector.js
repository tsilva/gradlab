...
export function rewardInspectionRows(history, selected, inspectedStep, gamma, limit = 5) {
  const points = new Map(history.map((point) => [point.step, point]));
  if (selected && history.length && selected.step >= history[0].step
      && selected.step <= history.at(-1).step) points.set(selected.step, selected);
  const candidates = [...points.values()].filter((point) => (
    point.step === selected?.step || point.step === inspectedStep
    || [point.reward_provider, point.reward_shaped].some((value) => Number.isFinite(value) && value !== 0)
  )).sort((a, b) => a.step - b.step);
  if (!candidates.length) return [];
  const focus = Number.isInteger(inspectedStep) ? inspectedStep : selected?.step;
  let nearest = 0;
  ...
  return candidates.slice(start, start + limit).map((point) => {
    const result = rewardContribution(point, selected?.step, gamma);
    const delay = Number.isInteger(selected?.step) ? point.step - selected.step : null;
...
export function createRewardInspector(legend, services) {
  ...
  table.setAttribute("aria-label", "Recorded rewards near the inspected step");
  const head = table.createTHead().insertRow();
  ["Step", "Native", "Shaped", "Delay (steps)", "Weight (γ^Δ)", "Contribution"].forEach((label) => {
  ...
  return {
    element: scroll,
    render(history, selected, inspectedStep, gamma) {
      const rows = rewardInspectionRows(history, selected, inspectedStep, gamma);
      const next = JSON.stringify([rows, selected?.step, gamma]);
      if (next === signature) return;
      signature = next;
      const focusedStep = table.contains(document.activeElement)
        ? document.activeElement.dataset.rewardStep : null;
      table.setAttribute("aria-description", `Up to five nearby recorded reward samples. Discounted shaped rewards from step ${selected?.step ?? "unavailable"}, gamma ${gamma ?? "unavailable"}. Past rewards are excluded. Discounting does not imply causation.`);

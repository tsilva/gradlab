...
export function createRenderSpeed(update) {
  const history = [];
  ...
  const reset = () => {
    ...
    clearWindow();
    history.length = 0;
    update({value: "— FPS", path: "", title: description()});
  };
  reset();
  ...
  const timer = setInterval(() => {
    ...
    const value = `${fps.toFixed(1)} FPS (${low.toFixed(0)}–${high.toFixed(0)})`;
    ...
    update({value, path: `M${left},32 ${points} L120,32 Z`, title: description()

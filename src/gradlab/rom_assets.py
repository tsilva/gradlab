...
DEFAULT_LOCAL_ROM_CACHE = Path("~/.cache/gradlab/roms").expanduser()
...
def rom_asset_state_path(repo_root: Path | None = None) -> Path:
    override = os.environ.get("GRADLAB_ROM_ASSET_STATE")
    if override:
        return Path(override).expanduser()
    if repo_root is not None:
        return repo_root / ".gradlab" / "rom-assets.json"
    return Path("~/.config/gradlab/rom-assets.json").expanduser()
...
def rom_asset_manifest_for_game(
    ...
    if not isinstance(value, Mapping):
        raise ValueError(
            f"ROM asset for {game!r} is not registered locally; run: gradlab rom sync --game {game}"
        )

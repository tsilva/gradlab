...
HEAD_ACTIVATIONS = frozenset({"tanh", "relu"})
...
def _normalize_encoder(value: Any, *, label: str) -> dict[str, Any]:
    ...
    if kind == "nature_cnn":
        ...
        if (
            not isinstance(features_dim, int)
            or isinstance(features_dim, bool)
            or features_dim <= 0
        ):
...
def _normalize_head(value: Any, *, label: str) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError(f"{label} must be an object")
    ...
    if activation not in HEAD_ACTIVATIONS:
        raise ValueError(f"{label}.activation must be one of {sorted(HEAD_ACTIVATIONS)}")
...
def normalize_policy_model(value: Any, *, label: str = "policy_model") -> dict[str, Any]:
    ...
    if value.get("schema_version") != POLICY_MODEL_SCHEMA_VERSION:
        raise ValueError(
            f"{label}.schema_version must be {POLICY_MODEL_SCHEMA_VERSION}"
        )
    ...
    if topology_kind not in POLICY_TOPOLOGIES:
        raise ValueError(
            f"{label}.topology.kind must be one of {sorted(POLICY_TOPOLOGIES)}"
        )
    if topology_kind == "shared_encoder":
        unexpected_topology = sorted(set(topology) - {"kind", "encoder"})
        if unexpected_topology:
            raise ValueError(
                f"{label}.topology has unexpected fields: {unexpected_topology}"
            )
        normalized_topology = {
            "kind": topology_kind,
        ...
        if not isinstance(encoders, Mapping) or set(encoders) != set(POLICY_ROLES):
            raise ValueError(
                f"{label}.topology.encoders must define exactly {list(POLICY_ROLES)}"
            )
        normalized_topology = {
            "kind": topology_kind,
            "encoders": {
    ...
    for raw_name, raw_encoder in raw_context_encoders.items():
        ...
        if unexpected_encoder:
            raise ValueError(
                f"{label}.context_encoders.{name} has unexpected fields: "
                f"{unexpected_encoder}"
            )
        kind = str(raw_encoder.get("kind") or "")
        if kind not in CONTEXT_ENCODERS:
            raise ValueError(
                f"{label}.context_encoders.{name}.kind must be one of "
                f"{sorted(CONTEXT_ENCODERS)}"
            )
    ...
    return {
        ...
        "fusion": fusion,
        "context_encoders": {
            name: context_encoders[name] for name in sorted(context_encoders)
        },
        "routes": {name: routes[name] for name in sorted(routes)},
        "heads": {
            role: _normalize_head(
                heads[role],
...
def validate_policy_model_context(
    ...
    *,
    ...
    if policy_model is None:
        return
    context_encoders = policy_model["context_encoders"]
    routes = policy_model["routes"]
    expected = set(fields)
    if set(context_encoders) != expected:
        raise ValueError(
            f"{label}.context_encoders must define exactly the task context fields "
            f"{sorted(expected)}"
        )
    if set(routes) != expected:
        raise ValueError(
            f"{label}.routes must define exactly the task context fields {sorted(expected)}"
        )
    for name, field in fields.items():
        source_kind = str(field["encoding"]["kind"])
        encoder_kind = str(context_encoders[name]["kind"])
        expected_encoder = "identity" if source_kind == "continuous" else "one_hot"
        if encoder_kind != expected_encoder:
            raise ValueError(

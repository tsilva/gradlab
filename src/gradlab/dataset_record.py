...
from gradlab.dataset_media import LosslessVideoWriter, iter_recoverable_video_prefix
from gradlab.dataset_providers import (
    EnvironmentArtifact,
    ProviderSession,
...
from gradlab.dataset_store import (
    adopt_tree,
    combine_trees,
    dataset_root,
...
class EpisodeJournal:
    ...
    def write_candidate(self, *, step: int, frame: np.ndarray) -> tuple[Path, str]:
        try:
            ...
            raise RuntimeError("Pillow is required; install GradLab with --extra dataset") from exc
...
def _current_tree_size(root: Path, reference: str) -> int:
    from gradlab.dataset_store import collection_paths
...
def _model_support_files(approved: ApprovedModelInput) -> dict[str, bytes]:
    ...
    model_payload = (
        ...
        if model is not None
            ...
            {
                "document_type": "gradlab.dataset.local_model",
                "format_version": 1,
    ...
    recipe_payload = (
        ...
        if recipe is not None
            ...
            {
                "document_type": "gradlab.dataset.explicit_environment",
                "format_version": 1,
            }
        )
    )
    ...
    document = {
        ...
        "runtime": {
            ...
            "packages": {
                ...
                for name in ("gradlab", "stable-baselines3", "torch", "numpy", session.provider_id)
            },
...
def _package_version(name: str) -> str | None:
    try:
        ...
        return __version__ if name == "gradlab" else None
    ...
    *,
    ...
    try:
        ...
        if agent == "human":
            ...
            from gradlab.play_web import WebHumanController
...
def reexecute_dataset(validation: Any) -> None:
    from gradlab.dataset_media import iter_episode_frames

    episodes = validation.dataset
    from gradlab.dataset_contract import grouped_episode_rows

...
from rlab.config_loader import RECIPE_TEMPLATE_VALUES, render_template_vars
...
from rlab.recipe_catalog import LOCAL_RUN_RECEIPT, recipe_identity, resolve_recipe_source
from rlab.recipe_documents import compose_train_document, prepare_checkpoint_eval_mode
from rlab.rom_assets import (
    DEFAULT_LOCAL_ROM_CACHE,
    rom_asset_manifest_for_game,
)
from rlab.seeds import DEFAULT_TRAIN_SEED
from rlab.train import INTERNAL_LEARNER_ENV


LOCAL_ROM_CACHE_ENV = "RLAB_ROM_CACHE_DIR"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--run-name",
        help=(
            "Relative output directory under --runs-dir. By default rlab creates a unique "
...
def _source_distribution() -> dict[str, str]:
    try:
        distribution = importlib.metadata.distribution("rlab")
    except importlib.metadata.PackageNotFoundError:
        return {"name": "rlab", "version": "uninstalled"}
    return {
        "name": str(distribution.metadata.get("Name") or "rlab"),
...
def _installed_source_commit() -> str | None:
    try:
        direct_url_text = importlib.metadata.distribution("rlab").read_text("direct_url.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    from rlab.train import main as learner_main
    ...
    print(
        f"play it: rlab play --recipe {source.reference!r} --runs-dir "

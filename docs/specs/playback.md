...
- The stop-condition editor must syntax-highlight supported expression tokens and make parse failures local: underline the authoritative error range and outline the expression field in red. Its autocomplete must escape the settings modal's scrolling container and selecting or scrolling suggestions must not dismiss Playback settings.

...
- Record every episode in the declared recording subset, selected independently of outcomes, with full unmasked lossless native RGB at contracted action cadence, initial image and true terminal image before reset.
...
- Preserve provider/shaped rewards, termination, truncation, interruptions, available timing, start facts, native score, shaped return, length, outcomes, absolute/normalized brick progress and its denominator in recordings; retain complete outcome/metric summaries for every evaluated episode, including unrecorded episodes.
...
- Emit final metrics only after every planned episode is complete and verified: success rate and two-sided 95% Wilson interval, mean/maximum/median normalized brick progress, mean native score, mean shaped return, mean episode length and completed count.
...
- Select the complete recorded episode nearest the full-set median normalized brick progress, breaking ties by manifest ordinal; record the rule, median, selected value and identity.
- When at least one episode is recorded, generate one full-episode video from the selected episode's original committed RGB frames at contracted cadence, without environment replay; store canonical bytes/hashes in R2 and deliver actual playable video plus full-set metrics through the sole W&B supervisor.
...
- Keep checkpoint monitoring disabled by default, including FirstWall PPO; explicit enablement requires its resolved configuration to be supported by calibration unless the operator explicitly opts into uncalibrated monitoring through a recorded Run override. The exception must preserve resource and compatibility checks and must not claim calibrated support or grant Acceptance. Tests may explicitly disable monitoring; this specification does not authorize live training or compute purchases.

## Explicit Publication and verification

- Transition datasets must declare a versioned contract for table structures, field meanings and relationships so consumers and publishers can detect incompatibilities before use.
- Dataset schema migrations must be explicit, identify source and target contracts, preserve source snapshots and provenance, and verify the resulting data.
- Use finalized verified Run inventories by default; explicit completed-snapshot publication may freeze only completed supervisor-verified evaluations from an active Run, excluding unfinished evaluations and without changing training. Use optional stage/performance predicates, excluding unrecorded episodes and filtering lightweight indexes before images and preserving complete multi-chunk episodes and grouping identities. Leave split assignment to consumers by default; an explicit request may publish a derived split view with reproducible assignments and preserved source revisions.
- Publish checkpoint recordings in the existing Breakout trajectories table format, including deduplicated lossless WebP frame assets, typed transition/episode/session records and brick annotations, while preserving the full HUD and making no downstream split assignment by default. Explicitly requested splits must preserve complete episodes, group reused evaluation seeds across Checkpoints, and report the achieved performance balance. Preserve unavailable source facts as missing rather than inventing values.
...
- A separately authorized bounded live campaign must cover at least three unique Checkpoints including final, full configured evaluation/recording sets, active contention and parallel final drain; inspect actual W&B video/charts in the in-app browser, remote integrity, cleanup and resource release. Live HF Publication requires an explicitly authorized target.

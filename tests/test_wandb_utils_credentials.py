...
from gradlab.operator_credentials import KeychainReference
...
""".strip()
    ...
    monkeypatch.setenv("GRADLAB_OPERATOR_CONFIG", str(config_path))
    monkeypatch.delenv("WANDB_API_KEY", raising=False)
    monkeypatch.delenv("WANDB_ENTITY", raising=False)
    monkeypatch.setattr(
        "gradlab.operator_credentials._keychain_lookup",
        lambda reference: (
            "wandb-key"
            if reference == KeychainReference("gradlab-wandb", "api-key")
            else None
        ),
    )

...
def _slow_history_worker(*args):
    ...
    original = PlaybackHost.chart_history
    ...
    with patch.object(PlaybackHost, "chart_history", blocked):
        _episode_inspection_worker(*args)


def test_slow_history_does_not_hold_control_rpc(tmp_path, monkeypatch):
    ...
    host = worker.IsolatedPlaybackHost(
        Namespace(fps=30, fixture_root=str(tmp_path)),
        argv=[],
        explicit_seed=False,
    )
    ...
    try:
        ...
        query = pool.submit(
            host.chart_history, snapshot["session_epoch"], snapshot["trajectory"]["episode_id"]
        )

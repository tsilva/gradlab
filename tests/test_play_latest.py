...
def test_latest_run_uses_creation_time_across_goals(monkeypatch):
    ...
    assert catalog.latest_run_route() == dict(
        level="runs",
        environment_id="two",
        goal_id="two",
        goal_variant_id="v2",
...
def test_latest_requires_catalog_authority():
    with pytest.raises(CatalogUnavailable):
        PlayCatalog().latest_run_route()


def test_latest_opens_exact_checkpoint(monkeypatch):
    authority = Mock()
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda url: (
            ...
            else dict(run_id=RUN_ID, checkpoints=[document])
        ),
    )
